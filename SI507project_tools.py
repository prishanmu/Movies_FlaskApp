...
class UpdateForm(Form):
    ...
    imdb_rating = FloatField('imdb rating', validators = [InputRequired()])
...
def new_movie(): #source: https://stackoverflow.com/questions/27367233/how-can-print-input-from-flask-wtform-to-html-with-sqlalchemy
    ...
    movie = Movie(title=form.movie.data, director_id=director.id, genre=form.genre.data, imdb_rating=form.imdb_rating.data)
    session.add(movie)
    session.commit()
    print('Movie Successfully Submitted!Check out "/all_movies" to see the whole list.')
...
def genre_ratings(genre):
    ...
    pic = ax.savefig("output.png")
    return render_template('rating_distribution.html') #CREATE HTML TEMPLATE
